...
// GetPlans returns no user-facing subscription packages.
// GET /api/v1/payment/plans
func (h *PaymentHandler) GetPlans(c *gin.Context) {
	// User-facing monthly subscription packages are intentionally hidden from the recharge flow.
	// Admin plan management and manual subscription assignment remain available.
	response.Success(c, []checkoutPlan{})
}
...
// payment methods with limits and recharge configuration.
...
func (h *PaymentHandler) GetCheckoutInfo(c *gin.Context) {
	...
	// User-facing monthly subscription packages are intentionally hidden from the recharge flow.
	// Keep the response shape stable while returning no purchasable plans.
	planList := make([]checkoutPlan, 0)
