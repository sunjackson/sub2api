...
type migrationRunnerConn interface {
...
func applyMigrationsFS(ctx context.Context, db *sql.DB, fsys fs.FS) (err error) {
	if db == nil {
		return errors.New("nil sql db")
	}
	...
	defer func() {
		if closeErr := conn.Close(); closeErr != nil {
			err = errors.Join(err, fmt.Errorf("close migrations connection: %w", closeErr))
		}
	}()
	...
	defer func() {
		...
		if unlockErr := pgAdvisoryUnlock(context.Background(), conn); unlockErr != nil {
...
func prepareNonTransactionalMigration(ctx context.Context, db migrationRunnerConn, name string) error {
	switch name {
...
func preparePaymentOrdersOutTradeNoUniqueMigration(ctx context.Context, db migrationRunnerConn) error {
...
func dropInvalidIndexIfPresent(ctx context.Context, db migrationRunnerConn, indexName string) error {
...
func findDuplicatePaymentOrderOutTradeNos(ctx context.Context, db migrationRunnerConn) ([]string, error) {
...
func indexIsInvalid(ctx context.Context, db migrationRunnerConn, indexName string) (bool, error) {
	var invalid bool
...
func ensureAtlasBaselineAligned(ctx context.Context, db migrationRunnerConn, fsys fs.FS) error {
...
func tableExists(ctx context.Context, db migrationRunnerConn, tableName string) (bool, error) {
	var exists bool
	err := db.QueryRowContext(ctx, `
		SELECT EXISTS (
			SELECT 1
...
func pgAdvisoryLock(ctx context.Context, db migrationRunnerConn) error {
...
func pgAdvisoryUnlock(ctx context.Context, db migrationRunnerConn) error {
	var unlocked bool
	...
	if !unlocked {
		return errors.New("release migrations lock: advisory lock was not held by this connection")
	}
	return nil
}
