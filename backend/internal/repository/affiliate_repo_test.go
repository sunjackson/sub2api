...
func TestAffiliateRecordQueriesUseLedgerAuditFields(t *testing.T) {
	...
	require.Contains(t, content, "ual.action = 'registration_reward'")
	require.Contains(t, content, "ual.action IN ('accrue', 'registration_reward')")
	require.Contains(t, content, "COALESCE(po.id, 0)")
	require.Contains(t, content, "ual.action,")
