export default {
  ...
  admin: {
    ...
    accountQuotaMonitor: {
      title: '额度监控',
      description: '监控中转平台账号余额和额度消耗',
      pageDescription: '按中转平台 Base URL 去重监控账号余额，自动跳过接口不可用的配置。',
      backToChannels: '返回渠道定价',
      provider: {
        custom: '其他/自动探测',
      },
      overview: {
        title: '账号 Base URL 一览',
        description: '按 Base URL 自动去重，同一家中转只保留一个余额监控；保存前会真实请求余额接口，失败不会纳入监控。',
        createMissing: '探测并补齐待监控',
        endpointGroups: 'Base URL 分组',
        coveredGroups: '已覆盖 / 待补齐',
        accountsWithEndpoint: '可识别账号',
        customGroups: '其他/自动探测',
        duplicates: '重复监控组',
        empty: '暂无可一览的账号 Base URL',
        customProbeHint: '其他，保存时自动探测',
        accounts: '账号数',
        monitors: '监控数',
        duplicateHint: '多余 {count} 个',
        covered: '已覆盖',
        createOne: '探测创建',
        createSuccess: '已通过真实余额接口验证并创建监控',
    ...
    accounts: {
      ...
      bulkDeleteFilteredConfirm: '未选择账号，将删除当前筛选出的 {count} 个账号。此操作无法撤销，是否继续？',
      ...
      quotaMonitor: {
        title: '余额监控',
        detectedProvider: '自动识别：{provider}',
        providerOther: '其他/自动探测',
        supportedHint: '开启后创建账号时自动加入额度监控，服务端复用该账号密钥真实查询余额；接口失败不会创建监控，也不会把密钥返回前端。',
        unsupportedHint: '需要先填写 Base URL；识别为其他时会使用通用余额接口自动探测，失败不会纳入监控。',
        noEndpoint: '账号已创建，但没有可用于余额监控的 Base URL',
        createFailed: '账号已创建，但自动创建余额监控失败',
        nameSuffix: '额度监控',
        duplicateSkipped: '该 Base URL 已有余额监控，本次不重复创建。',
      },
    ...
    ops: {
      ...
      alertRules: {
        ...
        metricDescriptions: {
          ...
          memory: '当前实例内存压力使用率（0~100，Total - MemAvailable 口径）。',
      ...
      tooltips: {
        ...
        memory: '内存压力口径：使用 Total - MemAvailable，Linux 页缓存/可回收缓存单独显示，缓存高不等于内存泄漏。',
