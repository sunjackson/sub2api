<template>
  <div class="mb-4 flex items-center justify-between rounded-lg bg-primary-50 p-3 dark:bg-primary-900/20">
    ...
    <div class="flex flex-wrap justify-end gap-2">
      ...
      <button @click="$emit('check-status')" class="btn btn-secondary btn-sm">{{ t('admin.accounts.bulkActions.checkStatus') }}</button>
...
defineProps(['selectedIds']); defineEmits(['delete', 'edit-selected', 'edit-filtered', 'clear', 'select-page', 'toggle-schedulable', 'reset-status', 'refresh-token', 'check-status']); const { t } = useI18n()
</script>
