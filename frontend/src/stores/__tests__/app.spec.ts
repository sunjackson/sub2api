...
describe('useAppStore', () => {
  ...
  describe('公开设置加载', () => {
    ...
    it('轻量注入配置后普通 fetchPublicSettings 不再强制拉完整配置', async () => {
      const windowAny = window as any
      windowAny.__APP_CONFIG__ = {
      ...
      expect(loaded?.site_name).toBe('Injected Site')
      expect(store.cachedPublicSettings?.home_content).toBe('')
      expect(store.siteLogo).toBe('')
      expect(store.publicSettingsPartial).toBe(true)
    })

    it('fetchCompletePublicSettings 会按需拉取完整配置', async () => {
      const windowAny = window as any
      windowAny.__APP_CONFIG__ = {
        registration_enabled: true,
        email_verify_enabled: false,
      ...
      const loaded = await store.fetchCompletePublicSettings()
      ...
      expect(store.publicSettingsPartial).toBe(false)
    })
